...
async function requestAndroidAlarmAccessIfNeeded() {
  ...
  // Android treats exact alarms as separate special access from notification
  // permission. Opening this screen is the platform-supported request flow.
  try {
    await IntentLauncher.startActivityAsync(IntentLauncher.ActivityAction.REQUEST_SCHEDULE_EXACT_ALARM);
  } catch {
    // Some vendor Android builds do not expose the dedicated screen. The
    // subsequent schedule call is authoritative and will surface a useful error.
  }
}
...
export async function enableNotifications(): Promise<{ enabled: boolean; reason?: string }> {
  ...
  try {
    await reconcileMaintenanceNotifications();
  } catch (e) {
    await AsyncStorage.removeItem(ENABLED_KEY);
    await cancelGloveboxNotifications();
    return {
      enabled: false,
      reason: e instanceof Error ? e.message : 'Could not schedule maintenance reminders on this device.',
    };
  }
  return { enabled: true };
}
...
export async function cancelGloveboxNotifications() {
  ...
  await Promise.all(
    scheduled
      .filter((n) => n.content.data?.source === 'glovebox')
      .map((n) => Notifications.cancelScheduledNotificationAsync(n.identifier)),
  );
}

function localNineAm(dateIso: string): Date {
  const [y, m, d] = dateIso.split('-').map(Number);
  return new Date(y!, m! - 1, d!, 9, 0, 0, 0);
}

async function readSent(workspace: string): Promise<Record<string, string>> {
  try {
    return JSON.parse((await AsyncStorage.getItem(`${SENT_KEY_PREFIX}${workspace}`)) ?? '{}') as Record<string, string>;
  } catch {
    return {};
  }
}

/** Rebuild scheduled date reminders and emit mileage/overdue notices only once per due target. */
export async function reconcileMaintenanceNotifications() {
  ...
  for (const reminder of reminders) {
    ...
    const mileageKey = `m:${reminder.id}`;
    const dateKey = `d:${reminder.id}`;

    if (mileageReached && sent[mileageKey] !== signature) {
      await Notifications.scheduleNotificationAsync({
        content: {
          title: 'Maintenance due',
          body: `${bodyBase}${reminder.dueMileage != null ? ` · ${reminder.dueMileage.toLocaleString()} mi` : ''}`,
          data: { source: 'glovebox', vehicleId: vehicle.id, url: `/vehicle/${vehicle.id}` },
        },
        trigger: null,
      });
      sent[mileageKey] = signature;
    }

    if (dateReached && sent[dateKey] !== signature) {
      await Notifications.scheduleNotificationAsync({
        content: {
          title: 'Maintenance due',
          body: bodyBase,
