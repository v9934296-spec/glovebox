...
export async function scanReceipt(localImageUri:string):Promise<ReceiptScan>{const context=ImageManipulator.manipulate(localImageUri);context.resize({width:1600,height:null});const rendered=await context.renderAsync();const saved=await rendered.saveAsync({compress:0.72,format:ImageManipulator.SaveFormat.JPEG});const file=new File(saved.uri);if((file.size??0)>4_500_000)throw new Error('Receipt image is still too large after compression. Crop it and try again.');const imageBase64=await file.base64();return parseReceiptScan(await invokeAi({task:'scan_receipt',imageBase64,mimeType:'image/jpeg'}));}
